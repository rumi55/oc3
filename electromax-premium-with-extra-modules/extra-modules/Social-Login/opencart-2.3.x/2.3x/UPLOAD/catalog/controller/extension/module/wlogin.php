...
class ControllerExtensionModulewlogin extends Controller {
	public function index(){
		...
		//FACEBOOK Start
		$data['flogin'] = $this->url->link('extension/module/wlogin/facebook','','SSL');
		//FACEBOOK END
		...
		$data['layouts']=array();
		if($this->config->get('wlogin_fstatus')){
			$data['layouts'][]=true;
		}
		if($this->config->get('wlogin_gstatus')){
			$data['layouts'][]=true;
		}
		if($this->config->get('wlogin_lstatus')){
			$data['layouts'][]=true;
		}
		if($this->config->get('wlogin_tstatus')){
			$data['layouts'][]=true;
		}
		...
		if(!$this->customer->isLogged()) {
			return $this->load->view('extension/module/wlogin', $data);
		}
	}
	...
	public function twitter() {
		...
		if (!empty($_GET['oauth_verifier']) && !empty($_SESSION['oauth_token']) && !empty($_SESSION['oauth_token_secret'])) {
			...
			if (isset($user_info->error)) {
				...
				$customer_infos = $this->model_module_wlogin->checkUserBySocialMedia('twitter',$twitterdata);
					if(!empty($customer_infos['customer']) && $customer_infos['customer'] == 'register'){
						...
						if(!empty($customer_info['approved'])){
							...
							$this->response->redirect($this->url->link('extension/module/wlogin', '&token=' . $token));
						}else{
				...
				$request_token = $twitteroauth->getRequestToken($this->url->link('extension/module/wlogin/twitter'));
