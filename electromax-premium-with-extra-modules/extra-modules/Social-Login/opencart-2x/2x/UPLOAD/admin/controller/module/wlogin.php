...
class ControllerModuleWlogin extends Controller {
	...
	public function index(){
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/wlogin', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('module/wlogin', 'token=' . $this->session->data['token'], true);
		...
		$data['wlogin_fcallback'] = $server.'index.php?route=module/wlogin/fcallback';
		// End 
		...
		$data['wlogin_gcallback'] = $server.'index.php?route=module/wlogin/gcallback';
		...
		$data['wlogin_lcallback'] = $server.'index.php?route=module/wlogin/lcallback';
		...
		$data['wlogin_tcallback'] = $server.'index.php?route=module/wlogin/twitter';
		...
		if(VERSION >= '2.2.0.0'){
			$this->response->setOutput($this->load->view('module/wlogin', $data));
		}else{
			$this->response->setOutput($this->load->view('module/wlogin.tpl', $data));
		}
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/wlogin')) {
