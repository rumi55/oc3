...
class ControllerModuleFilterProduct extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/filter_product', 'token=' . $this->session->data['token'], true));			
		}
		...
		$data['action'] = $this->url->link('module/filter_product', 'token=' . $this->session->data['token'], true);
		...
		$this->response->setOutput($this->load->view('module/filter_product', $data));
	}
