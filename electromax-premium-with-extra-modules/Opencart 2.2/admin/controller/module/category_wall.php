...
class ControllerModuleCategoryWall extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/category_wall', 'token=' . $this->session->data['token'], true));
		}
		...
		$data['action'] = $this->url->link('module/category_wall', 'token=' . $this->session->data['token'], true);
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Category Wall',
			'href' => $this->url->link('module/category_wall', 'token=' . $this->session->data['token'], true)
		);
		...
		$this->response->setOutput($this->load->view('module/category_wall', $data));
	}
