...
class ControllerModuleRevolutionSlider extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/revolution_slider', 'token=' . $this->session->data['token'], true));
		}
		...
		$data['action'] = $this->url->link('module/revolution_slider', 'token=' . $this->session->data['token'], true);
		$data['placement'] = $this->url->link('module/revolution_slider', 'token=' . $this->session->data['token'], true);		
		$data['existing'] = $this->url->link('module/revslideropencart', 'token=' . $this->session->data['token'], true);		
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/revolution_slider', 'token=' . $this->session->data['token'], true)
		);
		...
		$this->response->setOutput($this->load->view('module/revolution_slider/placement', $data));
	}
