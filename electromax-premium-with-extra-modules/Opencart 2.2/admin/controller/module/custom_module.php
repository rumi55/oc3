...
class ControllerModuleCustomModule extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/custom_module', 'token=' . $this->session->data['token'], true));
		}
		...
		$data['action'] = $this->url->link('module/custom_module', 'token=' . $this->session->data['token'], true);
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Custom module',
			'href' => $this->url->link('module/custom_module', 'token=' . $this->session->data['token'], true)
		);
		...
		$this->response->setOutput($this->load->view('module/custom_module', $data));
	}
