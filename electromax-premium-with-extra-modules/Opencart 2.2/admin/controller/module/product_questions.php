...
class ControllerModuleProductQuestions extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/product_questions', 'token=' . $this->session->data['token'], true));
		}
		...
		$data['action'] = $this->url->link('module/product_questions', 'token=' . $this->session->data['token'], true);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/product_questions', 'token=' . $this->session->data['token'], true)
		);
		...
		$this->response->setOutput($this->load->view('module/product_questions', $data));
	}
