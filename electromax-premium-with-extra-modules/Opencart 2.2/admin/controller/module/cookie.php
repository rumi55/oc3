...
class ControllerModuleCookie extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/cookie', 'token=' . $this->session->data['token'], true));
		}
		...
		$data['action'] = $this->url->link('module/cookie', 'token=' . $this->session->data['token'], true);
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Cookie',
			'href' => $this->url->link('module/cookie', 'token=' . $this->session->data['token'], true)
		);
		...
		$this->response->setOutput($this->load->view('module/cookie', $data));
	}
