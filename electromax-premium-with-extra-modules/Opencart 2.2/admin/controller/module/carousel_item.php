...
class ControllerModuleCarouselItem extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/carousel_item', 'token=' . $this->session->data['token'], true));
		}
		...
		$data['action'] = $this->url->link('module/carousel_item', 'token=' . $this->session->data['token'], true);
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Carousel item',
			'href' => $this->url->link('module/carousel_item', 'token=' . $this->session->data['token'], true)
		);
		...
		$this->response->setOutput($this->load->view('module/carousel_item', $data));
	}
