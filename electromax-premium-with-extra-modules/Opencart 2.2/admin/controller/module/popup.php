...
class ControllerModulePopup extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/popup', 'token=' . $this->session->data['token'], true));
		}
		...
		$data['action'] = $this->url->link('module/popup', 'token=' . $this->session->data['token'], true);
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Popup',
			'href' => $this->url->link('module/popup', 'token=' . $this->session->data['token'], true)
		);
		...
		$this->response->setOutput($this->load->view('module/popup', $data));
	}
