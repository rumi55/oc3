...
class ControllerModuleCookie extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/cookie', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		$data['action'] = $this->url->link('module/cookie', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Cookie',
			'href' => $this->url->link('module/cookie', 'token=' . $this->session->data['token'], 'SSL')
		);
		...
		$this->response->setOutput($this->load->view('module/cookie.tpl', $data));
	}
	
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/cookie')) {
