...
class ControllerModuleProductBlocks extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/product_blocks', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		$data['action'] = $this->url->link('module/product_blocks', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Product Blocks',
			'href' => $this->url->link('module/product_blocks', 'token=' . $this->session->data['token'], 'SSL')
		);
		...
		$this->response->setOutput($this->load->view('module/product_blocks.tpl', $data));
	}
