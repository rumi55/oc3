...
class ControllerModuleAdvancedGrid extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/advanced_grid', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		$data['action'] = $this->url->link('module/advanced_grid', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Advanced Grid',
			'href' => $this->url->link('module/advanced_grid', 'token=' . $this->session->data['token'], 'SSL')
		);
		...
		$this->response->setOutput($this->load->view('module/advanced_grid.tpl', $data));
	}
