...
class ControllerModuleMegaMenuManagerLinks extends Controller {
	
	private $error = array(); 
	
	public function link() {
		$this->language->load('module/megamenu_manager_links');
		...
		if(isset($_GET['link_id'])) {
			...
			if($data == false) { 
				$this->response->redirect($this->url->link('module/megamenu_manager_links', 'token=' . $this->session->data['token'], 'SSL'));
			}
			$data['name'] = $data['name'];
			$data['url'] = $data['url'];
		...
		$data['action'] = $this->url->link('module/megamenu_manager_links/link', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/megamenu_manager_links', 'token=' . $this->session->data['token'], 'SSL')
		);
		...
		$this->response->setOutput($this->load->view('module/megamenu_manager_links_edit.tpl', $data));
	...
	public function index() {  
		...
		$data['link_url'] = $this->url->link('module/megamenu_manager_links/link', 'token=' . $this->session->data['token'], 'SSL');	
		
		$data['action'] = $this->url->link('module/megamenu_manager_links', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('module/megamenu_manager_links.tpl', $data));
	}
	
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/megamenu_manager_links')) {
